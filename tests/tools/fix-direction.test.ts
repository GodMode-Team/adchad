...
describe('fix — stops the before/after one-trick-pony, keeps it simple', () => {
  it('the copy prompt tells the model not to default to before/after and to keep it uncluttered', () => {
    const p = buildFixCopyPrompt(look, 'Acme', 'this ad is generic garbage')
    expect(p).toMatch(/not default|overused|don.?t default/i)
    expect(p).toMatch(/before.?after/i) // it names the pattern it's de-emphasising
