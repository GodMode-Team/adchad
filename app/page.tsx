    ...
    <div style={{ background: BG, overflowX: 'hidden' }}>
      ...
      <div id="hero" style={{ background: GREEN, borderBottom: `4px solid ${INK}`, position: 'relative', overflow: 'hidden' }}>
        <div style={{ ...wrap, padding: '62px 22px 70px', display: 'flex', gap: 30, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 'none', width: 470, maxWidth: '100%', position: 'relative', alignSelf: 'center' }}>
            <video
              src="/adchad-demo.mp4"
              poster="/adchad-poster.jpg"
              controls playsInline muted autoPlay loop preload="metadata"
              aria-label="Watch Chad run the AI ad agency"
              style={{ width: '100%', display: 'block', borderRadius: 14, border: `4px solid ${INK}`, boxShadow: `8px 8px 0 ${PINK}`, background: '#000' }}
            />
            <div style={{ position: 'absolute', top: -16, right: -10, transform: 'rotate(9deg)', background: INK, color: YELLOW, fontFamily: F_BUNGEE, fontSize: 13, padding: '6px 10px', boxShadow: `4px 4px 0 ${PINK}`, animation: 'floaty 4s ease-in-out infinite', zIndex: 2 }}>WATCH ME<br /><span style={{ fontSize: 9, color: '#fff' }}>run the agency</span></div>
            <div style={{ position: 'absolute', bottom: -16, left: -16, transform: 'rotate(-8deg)', background: YELLOW, color: INK, fontFamily: F_MARKER, fontSize: 16, padding: '5px 11px', border: `3px solid ${INK}`, zIndex: 2 }}>🔊 sound on</div>
          </div>
        </div>
      </div>
