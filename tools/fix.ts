...
export async function fix(opts: { image: string; brand?: string | null }): Promise<FixResult> {
  if (!opts.image) throw new Error('fix: --image (the ad to fix) is required')
  const brand = opts.brand || 'this business'

  // 1. SEE the ad + its real flaws
  const look = await describe(opts.image)

  // 2. write copy that REPAIRS each flaw
  const prompt =
    ...
    `Its REAL flaws: ${(look.real_flaws ?? []).join('; ') || 'generic, weak hook'}.\n` +
    `Write a FIXED single-image ad that REPAIRS EACH flaw — add a clear CTA if missing, add proof/results, ` +
    `surface any buried offer, kill generic-template vibes, sharpen the hook. ` +
    `Return ONLY minified JSON {"headline","body","cta","direction"} where direction describes the ideal visual.`
