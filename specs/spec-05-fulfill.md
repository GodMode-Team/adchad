...
- **$5 fix:** `copy` skill rewrites headline/body/CTA → `creative` generates a ready-to-run ad image → `email send` delivers it → mark `fixes` + order fulfilled.
...
1. Fire a real test-mode `checkout.session.completed` → a `fixes` row with non-empty copy **and** a real `image_url`; buyer gets a real Resend email with the image.
